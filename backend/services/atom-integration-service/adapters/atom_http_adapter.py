...
========================================================
Driven Adapter que implementa PuertoArchivoPatrimonial comunicandose
con la API REST del sistema AtoM (Access to Memory).
Actua como Anti-Corruption Layer: transforma las respuestas JSON de AtoM
...
class AtoMHttpAdapter(PuertoArchivoPatrimonial):
    """
    Implementacion concreta del puerto PuertoArchivoPatrimonial.
    ...
    async def buscar_por_lenguaje_natural(self, query: str, limite: int = 5) -> list[DocumentoPatrimonial]:
        """
        Consulta el endpoint de busqueda de AtoM y mapea los resultados
        ...
        try:
            ...
            print(f"[AtoMHttpAdapter] Error en busqueda: {e}")
            return []

    async def obtener_documento_por_codigo(self, codigo: str) -> Optional[DocumentoPatrimonial]:
        """
        Obtiene un documento individual por su slug o codigo de referencia.
        """
        try:
            res = await self._http_client.get(
    ...
    def _mapear_desde_atom(data: dict) -> DocumentoPatrimonial:
        ...
        de dominio DocumentoPatrimonial. Centraliza toda la logica de mapeo
        en un unico punto para facilitar el mantenimiento ante cambios en
        la API externa.
        """
        # Extraer codigo de referencia desde dc:identifier
